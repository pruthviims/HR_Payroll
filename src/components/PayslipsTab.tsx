...
import { Download, FileDown, Search, Eye, X, Loader2, Calendar, FolderArchive, Palette } from 'lucide-react';
...
export const PayslipsTab: React.FC<PayslipsTabProps> = ({ employees, companyLogo, companyName, fieldConfigs }) => {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<PayslipTemplateId>(PayslipTemplateId.CLASSIC);
  ...
  const handleViewPdf = (emp: EmployeeSalaryData) => {
    ...
    setTimeout(() => {
      try {
        const doc = generateSinglePayslip(emp, companyLogo, companyName, fieldConfigs, false, selectedTemplate);
        const pdfBlob = doc.output('blob');
        const pdfUrl = URL.createObjectURL(pdfBlob);
        setPreviewPdfUri(pdfUrl);
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm">
        <div className="flex items-center gap-6">
          {companyLogo && (
            <div className="w-16 h-12 hidden md:block">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 mr-4 hidden lg:flex bg-gray-50 px-4 py-2 rounded-2xl border border-gray-100">
            <Palette size={16} className="text-indigo-500" />
            <select 
              value={selectedTemplate} 
              onChange={(e) => setSelectedTemplate(e.target.value as PayslipTemplateId)}
              className="text-[10px] font-black text-gray-700 bg-transparent outline-none uppercase tracking-widest cursor-pointer"
            >
              <option value={PayslipTemplateId.CLASSIC}>Classic Style</option>
              <option value={PayslipTemplateId.MODERN}>Modern Indigo</option>
              <option value={PayslipTemplateId.PROFESSIONAL}>Professional Blue</option>
              <option value={PayslipTemplateId.MINIMAL}>Minimalist Text</option>
            </select>
          </div>
          <button 
            onClick={async () => { setIsZipping(true); await generateIndividualZippedPayslips(filteredEmployees, companyLogo, companyName, fieldConfigs, selectedTemplate); setIsZipping(false); }} 
          ...
          <button 
            onClick={() => generateBulkPayslips(filteredEmployees, companyLogo, companyName, fieldConfigs, selectedTemplate)} 
          ...
          {filteredEmployees.map((emp) => (
            <div key={`${emp.id}-${emp.month}-${emp.year}`} className="bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-xl transition-all flex flex-col group">
              ...
              <div className="grid grid-cols-2 gap-3 mt-auto">
                ...
                <button onClick={() => generateSinglePayslip(emp, companyLogo, companyName, fieldConfigs, true, selectedTemplate)} className="flex items-center justify-center gap-2 py-3.5 bg-indigo-50 text-indigo-600 font-black rounded-2xl border border-indigo-100 text-[10px] uppercase tracking-widest shadow-sm shadow-indigo-100/50 hover:bg-indigo-100 transition-colors">
                  <Download size={16} />
                  Get PDF
                </button>
              </div>
          ...
          <div className="bg-white w-full max-w-6xl h-full max-h-[90vh] rounded-[40px] shadow-3xl overflow-hidden flex flex-col">
            <div className="p-8 border-b border-gray-100 bg-white flex items-center justify-between">
              <div className="flex items-center gap-4">
                {companyLogo && <img src={companyLogo} alt="Logo" className="w-12 h-10 object-contain" />}
